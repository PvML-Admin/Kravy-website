...
function MemberManagement() {
  ...
  // Search for members to manage booster status
  const handleBoosterSearch = async (term) => {
    setBoosterSearchTerm(term);
  ...
  const handleToggleBooster = async (member) => {
    try {
      const newStatus = !member.is_discord_booster;
      await membersAPI.toggleDiscordBooster(member.id, newStatus);
      
      setMessage({
        type: 'success',
        text: `${member.display_name || member.name} ${newStatus ? 'is now' : 'is no longer'} a Discord Booster!`
      });
      
      // Refresh search results
      if (boosterSearchTerm.trim()) {
        handleBoosterSearch(boosterSearchTerm);
      }
    } catch (err) {
      setMessage({ type: 'error', text: err.response?.data?.error || err.message });
    }
  ...
  return (
    <div>
      ...
      <div className="card" style={{ marginTop: '20px' }}>
        <h2>Discord Booster Management</h2>
        <p style={{ fontSize: '0.9rem', color: '#999', marginBottom: '15px' }}>
          Search for members and toggle their Discord Booster status. Boosters will appear with a special icon and hot pink name.
        </p>
        ...
        <div className="form-group">
          ...
          <input
            type="text"
            className="form-control"
            value={boosterSearchTerm}
            onChange={(e) => handleBoosterSearch(e.target.value)}
            ...
            {searchResults.map(member => (
              <div key={member.id} className="booster-result-item">
                <div className="booster-member-info">
                  <span className={`booster-member-name ${member.is_discord_booster ? 'is-booster' : ''}`}>
                    {member.is_discord_booster && (
                      <img 
                        src="/discord_booster.png" 
                        alt="Discord Booster" 
                        className="booster-icon-img"
                ...
                <button
                  type="button"
                  className={`btn ${member.is_discord_booster ? 'btn-danger' : 'btn-booster'}`}
                  onClick={() => handleToggleBooster(member)}
                >
                  {member.is_discord_booster ? 'Remove Booster' : 'Add Booster'}
                </button>
              </div>
            ))}
          </div>
