// RuneScape skill max levels and metadata

const SKILL_MAX_LEVELS = {
  // === 99 MAX SKILLS ===
  // Combat
  'Attack': 99,
  'Strength': 99,
  'Defence': 99,
  'Constitution': 99,
  'Ranged': 99,
  'Prayer': 99,
  'Magic': 99,
  
  // Gathering & Production
  'Fishing': 99,
  'Cooking': 99,
  
  // Support
  'Agility': 99,
  'Thieving': 99,
  'Hunter': 99,
  'Construction': 99,
  'Summoning': 99,
  'Divination': 99,
  
  // === 110 MAX SKILLS ===
  'Mining': 110,
  'Smithing': 110,
  'Crafting': 110,
  'Firemaking': 110,
  'Fletching': 110,
  'Woodcutting': 110,
  'Runecrafting': 110,
  
  // === 120 MAX SKILLS ===
  'Dungeoneering': 120,
  'Herblore': 120,
  'Slayer': 120,
  'Farming': 120,
  'Invention': 120,
  'Archaeology': 120,
  'Necromancy': 120
};

/**
 * Get the maximum level for a skill
 */
function getSkillMaxLevel(skillName) {
  return SKILL_MAX_LEVELS[skillName] || 99;
}

/**
 * Get skill category
 */
function getSkillCategory(skillName) {
  const maxLevel = getSkillMaxLevel(skillName);
  if (maxLevel === 120) return '120';
  if (maxLevel === 110) return '110';
  return '99';
}

/**
 * Calculate percentage to max level
 */
function getSkillProgress(skillName, currentLevel, currentXp) {
  const maxLevel = getSkillMaxLevel(skillName);
  const maxXp = getXpForLevel(maxLevel);
  
  return {
    levelProgress: (currentLevel / maxLevel) * 100,
    xpProgress: (currentXp / maxXp) * 100,
    maxLevel,
    isMaxed: currentLevel >= maxLevel
  };
}

/**
 * XP table for levels 1-120
 * Source: RuneScape Wiki
 */
const XP_TABLE = [
  0, 83, 174, 276, 388, 512, 650, 801, 969, 1154, // 1-10
  1358, 1584, 1833, 2107, 2411, 2746, 3115, 3523, 3973, 4470, // 11-20
  5018, 5624, 6291, 7028, 7842, 8740, 9730, 10824, 12031, 13363, // 21-30
  14833, 16456, 18247, 20224, 22406, 24815, 27473, 30408, 33648, 37224, // 31-40
  41171, 45529, 50339, 55649, 61512, 67983, 75127, 83014, 91721, 101333, // 41-50
  111945, 123660, 136594, 150872, 166636, 184040, 203254, 224466, 247886, 273742, // 51-60
  302288, 333804, 368599, 407015, 449428, 496254, 547953, 605032, 668051, 737627, // 61-70
  814445, 899257, 992895, 1096278, 1210421, 1336443, 1475581, 1629200, 1798808, 1986068, // 71-80
  2192818, 2421087, 2673114, 2951373, 3258594, 3597792, 3972294, 4385776, 4842295, 5346332, // 81-90
  5902831, 6517253, 7195629, 7944614, 8771558, 9684577, 10692629, 11805606, 13034431, 14391160, // 91-100
  15889109, 17542976, 19368992, 21385073, 23611006, 26068632, 28782069, 31777943, 35085654, 38737661, // 101-110
  42769801, 47221641, 52136869, 57563718, 63555443, 70170840, 77474828, 85539082, 94442737, 104273167, // 111-120
  115126838, 127110260, 140341028, 154948977, 171077457, 188884740, 200000000 // 121-127 (virtual levels)
];

/**
 * Get XP required for a specific level
 */
function getXpForLevel(level) {
  if (level < 1 || level > XP_TABLE.length) return 0;
  return XP_TABLE[level - 1];
}

/**
 * Get level from XP (returns actual level, capped at skill max)
 */
function getLevelFromXp(xp, maxLevel = 99) {
  let level = 1;
  for (let i = XP_TABLE.length; i >= 1; i--) {
    if (xp >= XP_TABLE[i - 1]) {
      level = i;
      break;
    }
  }
  // Cap at the skill's max level
  return Math.min(level, maxLevel);
}

/**
 * Get virtual level from XP (can go beyond skill cap, up to 120)
 */
function getVirtualLevelFromXp(xp) {
  for (let level = XP_TABLE.length; level >= 1; level--) {
    if (xp >= XP_TABLE[level - 1]) {
      return level;
    }
  }
  return 1;
}

/**
 * Get XP to next level
 */
function getXpToNextLevel(currentLevel, currentXp) {
  if (currentLevel >= XP_TABLE.length) return 0;
  const nextLevelXp = getXpForLevel(currentLevel + 1);
  return nextLevelXp - currentXp;
}

/**
 * Calculate percentage to next level
 */
function getPercentageToNextLevel(currentLevel, currentXp) {
  if (currentLevel >= XP_TABLE.length) return 100;
  
  const currentLevelXp = getXpForLevel(currentLevel);
  const nextLevelXp = getXpForLevel(currentLevel + 1);
  const xpInLevel = currentXp - currentLevelXp;
  const xpNeeded = nextLevelXp - currentLevelXp;
  
  return (xpInLevel / xpNeeded) * 100;
}

module.exports = {
  SKILL_MAX_LEVELS,
  getSkillMaxLevel,
  getSkillCategory,
  getSkillProgress,
  getXpForLevel,
  getLevelFromXp,
  getVirtualLevelFromXp,
  getXpToNextLevel,
  getPercentageToNextLevel
};

